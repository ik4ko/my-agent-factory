...
/**
 * Inbound voice webhook â€” maps a provider transcript to the same
 * parseCommand/runCommand core the SMS path and /api/comms/simulate use.
 ...
 * each have their own webhook shape and expect their own reply format) â€” this
...
export async function POST(req: NextRequest) {
  ...
  if (!vapiKey && !retellKey) {
    return NextResponse.json({ error: 'Voice not configured â€” set VAPI_API_KEY or RETELL_API_KEY' }, { status: 501 });
  }
