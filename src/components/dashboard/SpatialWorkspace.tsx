 ...
 *  cache (owned by MetricsBar's realtime subscription on this page). */
function TokenStream() {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {lanes.length === 0 ? (
        <p className="rounded-md border border-dashed border-border p-3 font-terminal text-[10px] text-muted-foreground/40">
          No model calls in the last 24h — dispatch from the Orchestrate pane to populate the stream.
        </p>
      ) : (
        <div className="overflow-x-auto rounded-md border border-border">
          <table className="w-full font-terminal text-[10px]">
            ...
            <tbody>
              {lanes.map((l) => (
                <tr key={l.model} className="border-b border-border/40 last:border-b-0">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        ...
        {room === 'analytics' && (
          <div className="flex flex-col gap-3">
            ...
            <a
              href="/dashboard/rooms/analytics"
              className="inline-flex w-fit items-center gap-1.5 rounded-md border border-neon-orange/40 bg-neon-orange/10 px-3 py-1.5 font-terminal text-[10px] text-neon-orange transition-colors hover:bg-neon-orange/20"
            >
              <Activity className="size-3" aria-hidden /> Full Analytics Room · dry-run PnL + loop throughput
            </a>
          </div>
        )}
      </div>
