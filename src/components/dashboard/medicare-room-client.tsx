...
import { ArrowRight, CircleAlert, Database, Mail, MessageSquare, RefreshCcw, Search, ShieldCheck, ShieldAlert, Smartphone, StickyNote, Upload } from 'lucide-react';
...
/**
 * The old import panel parsed a workbook in the browser and POSTed the rows
 * straight to `upsert(..., { onConflict: 'id' })` on ag_clients / ag_carriers /
 * ag_policies. A spreadsheet carrying an `id` column therefore overwrote live
 * records with no preview against existing values, no provenance, no audit
 * trail, and no way back. It has been withdrawn rather than left available
 * with a warning attached.
 *
 * Its replacement is the staged import batch: upload, map, compare against
 * what is already held, approve, commit — the same review gate the coverage
 * diffs already go through. Until that ships, this panel states plainly that
 * there is no import path, which is safer than one that quietly destroys data.
 */
function ImportPanel() {
  return (
    <PanelChrome
      title="IMPORT & INGESTION HUB"
      headerRight={<span className="text-[10px] text-muted-foreground">Withdrawn</span>}
      bodyClassName="space-y-3"
    >
      <div className="flex items-start gap-2 rounded border border-amber-400/30 bg-amber-400/10 p-3 text-xs text-amber-100">
        <ShieldAlert className="mt-0.5 size-4 shrink-0" />
        <div>
          <div className="font-semibold uppercase tracking-wider">Direct import disabled</div>
          <div className="mt-1">
            The previous spreadsheet import wrote straight onto live client and policy records — a file
            containing an <code>id</code> column silently overwrote whatever was already there.
          </div>
          <div className="mt-1.5">
            It is being replaced by a staged import: preview every row against the current value,
            approve the changes, then commit as one auditable batch. Coverage changes already work this
            way — see <span className="font-medium">Coverage Reviews</span>.
          </div>
        </div>
      </div>
      <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
        <Upload className="size-3.5" />
        Need to load a book of business now? Ask for the staged import to be brought forward.
      </div>
    </PanelChrome>
  );
}
